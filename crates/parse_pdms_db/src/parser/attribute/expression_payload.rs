...
pub fn scan_expression_payload_opcodes(input: &[u8]) -> Result<OpcodeScanReport, DecodeError> {
    ...
    for start_words in 0..=2 {
        let Ok((consumed_bytes, words, start_idx, declared_words)) =
            read_words_from_start(input, start_words)
        else {
            continue;
        };
...
fn decode_words(words: &[i32]) -> Result<String, DecodeError> {
    ...
    if stack.len() == 1 {
        Ok(stack
            .pop()
            .unwrap_or_else(|| ExprNode::new(String::new(), PREC_VALUE))
            .text)
    } else {
...
fn scan_words_for_opcodes(words: &[i32]) -> Result<(BTreeMap<i32, u64>, Vec<i32>), DecodeError> {
    ...
    Ok((counts, unknown.keys().copied().collect::<Vec<_>>()))
}

fn skip_value_opcode(opcode: i32, words: &[i32], idx: usize) -> Result<Option<usize>, DecodeError> {
...
mod tests {
    ...
    fn test_decode_expression_payload_attr_index_1() {
        let input: [u8; 76] = [
            0x1C, 0x00, 0x00, 0x12, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00,
            0x00, 0x01, 0x00, 0x00, 0x00, 0x65, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x40, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x06, 0x00, 0x00, 0x00, 0x6A, 0x00, 0x00, 0x00, 0x02, 0x00, 0x0D, 0x20, 0xC7,
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x06, 0x41, 0x00, 0x00, 0x06, 0xA5,
        ];
