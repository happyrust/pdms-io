...
impl PdmsIO {
    ...
    /// 返回一个历史参考号集合，值为所有的位置
    ///
    /// **specs/003 T302 分工声明（契约 D4）**：本函数为**全库历史回填专用入口**
    /// （含物理 offset 的逐会话全量收集,新增量入口 `update_elements_to_database`
    /// 不覆盖该场景），予以保留；其落库段仍为遗留字符串拼接(INSERT IGNORE),
    /// 已记 004 候选改造为强类型路径——增量主线禁止新增对它的依赖。
    #[cfg(feature = "surrealdb")]
    ...
    pub async fn collect_and_save_latest_data(
        ...
        // 第三步：保存到数据库（specs/003 T301：保存段委托唯一落库入口
        // update_elements_to_database —— 强类型 upsert 幂等 + 水位；
        // 旧 save_sessions_and_elements 字符串拼接路径已退役）
        println!("\n3. 保存数据到 SurrealDB(ses/pe_ses_h/pe + 水位)...");
        let save_start_time = Instant::now();
        self.update_elements_to_database(&range_eles, false).await?;
...
pub fn extract_test_refnos(io: &mut PdmsIO, count: usize) -> anyhow::Result<Vec<RefU64>> {
    ...
    if index_data.level == 0 {
        ...
        // specs/002 Phase 4：v1 `process_leaf_node` 的 4K 动态偏移单元测试随其删除
        //（语义由 build_index_map 委托路径内联保持,真实样本守护 = ams1112 512→2048）。
        return Ok(refnos);
    }
