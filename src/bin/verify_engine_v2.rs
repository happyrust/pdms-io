...
fn main() {
    ...
    match TableIterator::new(
        &mut db.cache, &mut db.handle,
        dbno, extent, root_pgno,
    ) {
        Ok(mut iter) => {
            match iter.collect_all(&mut db.cache, &mut db.handle) {
                Ok(entries) => {
                    println!("  索引条目数: {} (耗时 {:?})", entries.len(), t2.elapsed());
                    for e in entries.iter().take(5) {
                        println!("    RefNo({:#010X}:{:#010X}) → page={} offset={}",
                            e.refno.hi, e.refno.lo, e.page_no, e.offset());
                    }
                    if entries.len() > 5 {
                    ...
                    if let Some(first) = entries.first() {
                        ...
                        match db.find_element(target, root_pgno) {
                            Ok(Some(loc)) => {
                                println!("  搜索 {} → page={} offset={} (耗时 {:?})",
                                    target, loc.page_no, loc.offset, t3.elapsed());
                            }
