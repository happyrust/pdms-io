...
- **F1-I1**: 重组流 = 记录窗口字节 + rec[6]/rec[7](DA)与 rec[8]/rec[9](members)链按链序重组的节点 payload,布局对齐 `parse_pdms_db` 现行解析预期。
  ...
  **预算导向修订(2026-06-12,T202 实测回填)**:链尾节点可带 **slack 词**(节点头声明长度 > 预算余量;sam7200 (23584,5535) members 节点声明 16 词、预算余 12,slack 1 词)——e3d_io 解码以预算截取,v1 以节点头长合并 ⇒ slack 混入致 %8 失败、children 静默为空。重组流照 e3d_io 预算语义:**payload 仅取 `min(节点声明-5, 预算余量)` 词,节点头 low16 相应改写为 take+5**;无 slack 节点 = 逐字节原样。w1..w4(refno/链指针)恒原样。
