...
- [ ] T204 [P1] B 树/查找委托：`read_index_data`/`search_in_leaf_node`/`build_index_map*`/`search_latest_refno` 族 → `e3d_io` B 树（经 `PagedFile` 页源）—（**e3d_io 侧已备 2026-06-11**:`Rdb::btree_find` 目标式 O(log n) 点查,与 `btree_descend` 同式〔最右 separator<=key、ci=0 默认、哨兵 -inf、深度 40〕,sam7200 **10392 键穷举**双源等值 + 缺席键 None 测试通过;**io.rs 侧换线待做**:`btree_search_fixed`/`search_latest_refno` 族切到 `Rdb<PagedFile>`,需先对照 v1 RootIndexPage lower/upper 双根与 flag 语义做 parity 分析,以 workspace 既有测试为 oracle）
