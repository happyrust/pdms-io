...
- [x] T205 [P1] 元素读取委托：`parse_raw_element`/`auto_get_raw_element`/`read_element_record_cached` → `e3d_io` 记录解码；`EleData` 适配策略落地（FR-003：评估 `parse_pdms_db` 保留为类型适配层 or 并入）— **完成 2026-06-11**:变长记录读取(自适应 16K→64K 窗口 + 记录定界:隐式区顺延/0x0001 显式块/0x0002 成员块/0x07 追加段/双词终止/相邻记录启发)同式移植为 `Rdb::element_record`(纯函数定界 `record_end` 一族);`read_element_record_cached` 改为委托(签名冻结),`parse_raw_element`/`auto_get_raw_element`/`parse_element` 自动流经新芯。**FR-003 决策:`parse_pdms_db` 保留为 `EleData` 类型适配层**——字节获取已单源 e3d_io,`parse_raw_ele_data` 原样保证字段级兼容;并入会把 aios_core 类型依赖拖进 std-only crate(违宪法 I/隔离),不并入。parity 闸:`test_element_record_parity`(ams1112 全库 index map 抽样 ~430 条,v1 `ElementRecordReader` vs `Rdb::element_record` 逐字节相等)通过;lib 77/1〔既有〕/5,e3d_io 38+1。v1 `ElementRecordReader` 留作 parity 对照,Phase 3 T303 删除
- [x] T206 [P1] 增量/历史迁移：`collect_increment_eles` 族 / `collect_ele_history` / `get_refno_*_status` 跑在新芯上，现有断言不动（契约 C3.1）— **完成 2026-06-11**:底层字节路径全部换芯——`get_page_cached`/`read_data_cached`(及其上的 `read_index_data`/`find_refno_loc`/`get_attribute_value`/`read_bytes`)改经 `Rdb::slice`,`cache_hit_rate` 统计源切换为 Rdb 影子页命中(承接 v1 hit_rate 语义,C1 注"仍有意义"达成);v1 `PageManager`/`PagedReader` 退出全部读路径(仅余 pub 字段与自身测试,Phase 3 删),`local_file_ext_no` 标记退役(C2 I4 承接)。增量/历史的**编排逻辑与断言零改动**(`filter_index_data` 会话窗口过滤为冻结 API 编排,跑在新芯页上)。oracle:lib 77/1〔既有〕/5 与基线一致
- [x] T207 [P2] 索引缓存文件兼容决策落地（契约 C3.4 二选一），回填契约文档 — **决策 = (a) 磁盘格式逐字节兼容**:换芯只改了 IndexMap 的**构建来源**(e3d_io 枚举),`PIM1` v1 缓存格式(magic+version+page_size+count+entries)与旧无 magic 格式的读取兼容逻辑均未动;版本/页大小失配走**显式报错要求重建**(既有行为,非静默重建)。`test_index_map_cache_roundtrip_pim1` 持续护栏;契约 C3.4 已回填
- [x] T208 GATE：`cargo build --workspace` + 全部现有测试全绿（`src/test/`、`src/tests/`、`tests/`、bins 编译）；调用方零修改 —（**基线注记 2026-06-11**:`page_manager::tests::test_dirty_eviction_writes_back` 在换芯**之前**即失败〔git stash 复测确认〕,属 v1 写侧孤岛(T303 删除对象);T208 验收口径 = 不劣于该基线,或 Phase 3 先删除该孤岛后全绿）**通过 2026-06-11 18:14**:workspace 全量构建成功(全 bins/测试目标编译);lib 77 过/1 失(唯一失败=上述既有基线红灯)/5 忽略;集成测试**全绿**——api_freeze_c1 ✓、bend_angl_ams1112 ✓、dblist_regression ✓、debug_trim_lowcase 4✓、desp_ams1112 ✓、**diag_ams1112_full_parse 5✓(394s,103MB 全库解析+关键会话回归全程跑在新芯上)**、ptcd_surreal_sampling ✓;e3d_io 38+1 独立全绿(当日复验);调用方零修改、冻结锁未触发。**Phase 2 收口,Phase 3 解锁**
