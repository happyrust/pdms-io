...
- **R5 回声盲区(2026-06-11 T301 实测发现;✅ 已由 specs/005 修复 2026-06-12——增量路径换链式重组流,回声测试三类编辑齐发转正)**:**DA 文本编辑(Rename 等)的回声会被 v1 增量读取漏检**——e3d_io 的 DA 重定位把文本搬到文件尾新页(格式合法,001 链式解码读回正常),但 v1 `EleData` 解析(`parse_raw_ele_data` 路径)是**窗口邻接假设**(自记录偏移连续读 16~64K),看不见远页 DA ⇒ 改名被定性"无变化"而不进增量/不落库。**文件级真相不受影响**(写回核心读回核验已证);仅"回声进库"这一段对 DA 编辑失明。已界定为 v1 解析器既有局限(非 004 写回缺陷);**005 候选**:`EleData` 的 DA 区解析改走链式追页(e3d_io `Rdb::slice` 可支撑)。T301 回声测试据此限定内联属性编辑面。
