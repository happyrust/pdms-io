...
import { loadDiagramOptions, getLiveDir, loadDiagramSource, validatePreviewId, } from "./file-utils.js";
import { webLogger } from "./logger.js";
import { matchRoute } from "./routes.js";
import { CSP_HEADER, SERVER_PORT_START, SERVER_PORT_END, ROUTES, CONTENT_TYPES, CACHE_CONTROL, WS_MESSAGES, DEFAULT_DIAGRAM_OPTIONS, TEMPLATE_VARS, } from "./constants.js";
...
async function findAvailablePort(startPort = SERVER_PORT_START, maxPort = SERVER_PORT_END) {
...
async function handleViewRequest(url, res, port) {
...
async function handleLivePreviewRequest(url, res, port) {
...
async function handleMermaidLiveRequest(url, res) {
...
export async function ensureLiveServer() {
    ...
    liveServer = createServer(async (req, res) => {
        const url = req.url || "/";
        try {
...
export async function addLiveDiagram(diagramId, filePath) {
...
export function hasActiveConnections(diagramId) {
    const state = diagrams.get(diagramId);
...
function notifyClients(diagramId) {
    const state = diagrams.get(diagramId);
    if (!state)
        return;
    let notifiedCount = 0;
 ...
 */
export async function closeLiveServer() {
...
async function createLiveHtmlWrapper(content, diagramId, port, background = "white", liveEnabled = true) {
