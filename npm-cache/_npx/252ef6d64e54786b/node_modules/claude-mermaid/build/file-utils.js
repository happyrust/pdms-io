...
import { APP_NAME, PREVIEW_ID_REGEX, UNIX_SYSTEM_PATHS, WINDOWS_SYSTEM_PATHS, TIMEOUTS, FILE_NAMES, DIR_NAMES, } from "./constants.js";
 ...
 */
export function validatePreviewId(previewId) {
    if (!previewId || !PREVIEW_ID_REGEX.test(previewId)) {
        throw new Error("Invalid preview ID format. Only alphanumeric characters, hyphens, and underscores are allowed.");
    }
}
export function getPreviewDir(previewId) {
    validatePreviewId(previewId);
    return join(getLiveDir(), previewId);
}
export function getDiagramFilePath(previewId, format) {
    return join(getPreviewDir(previewId), `diagram.${format}`);
}
export function getDiagramSourcePath(previewId) {
    return join(getPreviewDir(previewId), FILE_NAMES.DIAGRAM_SOURCE);
}
export function getDiagramOptionsPath(previewId) {
...
export async function saveDiagramSource(previewId, diagram, options) {
...
export async function loadDiagramSource(previewId) {
    const sourcePath = getDiagramSourcePath(previewId);
    return await readFile(sourcePath, "utf-8");
}
export async function loadDiagramOptions(previewId) {
    const optionsPath = getDiagramOptionsPath(previewId);
 ...
 */
async function deleteDiagramDirectory(dirPath) {
...
export async function deleteDiagram(previewId) {
    validatePreviewId(previewId);
    ...
    try {
        ...
        throw new Error(`Failed to delete diagram: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 ...
 */
export function validateSavePath(savePath) {
...
export async function cleanupOldDiagrams(maxAgeMs = TIMEOUTS.CLEANUP_MAX_AGE_MS) {
    try {
