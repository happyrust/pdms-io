...
import { ROUTES, CONTENT_TYPES, CSP_HEADER, CACHE_CONTROL, TEMPLATE_FILES, ASSET_FILES, } from "./constants.js";
 ...
 */
async function serveStaticFile(filePath, contentType, cacheControl = CACHE_CONTROL.NO_STORE) {
    const content = await readFile(filePath, contentType.includes("text") ? "utf-8" : undefined);
    return { content: content, contentType, cacheControl };
}
/**
 * Gallery Page Handler
 * Renders the main gallery page
 */
export async function handleGallery(context) {
    const { res, port } = context;
    try {
        webLogger.debug("Gallery page request");
        const html = await renderPage(TEMPLATE_FILES.GALLERY, { PORT: port }, {
 ...
 */
export async function handleApiDiagrams(context) {
    const { res } = context;
    try {
 ...
 */
export async function handleApiDiagramDelete(context) {
 ...
 */
export async function handleSharedCss(context) {
    const { res } = context;
    try {
        const { content, contentType, cacheControl } = await serveStaticFile(join(PREVIEW_DIR, ASSET_FILES.SHARED_STYLE), CONTENT_TYPES.CSS);
        res.writeHead(200, {
...
export async function handleGalleryCss(context) {
    const { res } = context;
    try {
        const { content, contentType, cacheControl } = await serveStaticFile(join(PREVIEW_DIR, ASSET_FILES.GALLERY_STYLE), CONTENT_TYPES.CSS);
        res.writeHead(200, {
            "Content-Type": contentType,
            "Cache-Control": cacheControl,
        });
...
export async function handleGalleryJs(context) {
    const { res } = context;
    try {
        const { content, contentType, cacheControl } = await serveStaticFile(join(PREVIEW_DIR, ASSET_FILES.GALLERY_SCRIPT), CONTENT_TYPES.JAVASCRIPT);
        res.writeHead(200, {
            "Content-Type": contentType,
            "Cache-Control": cacheControl,
        });
