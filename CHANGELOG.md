...
### 新增 — e3d-syncd 常驻同步守护(specs/006,2026-06-12)

> 把 001~005 的全部能力串成生产形态:**文件一变,库自动跟上**。grill Q0~Q6 全按推荐(纯水位驱动/静定窗+轮询兜底/只读链路写回自动化留 007)。

- **同步核心** `sync_core`:`scan_targets`(`*_0001` 识别+64B 读头+白名单)+ `sync_db`(纯水位驱动:跳过/范围增量经唯一入口/初见库立基线不回灌/Failed 隔离;零本地状态=重启安全免费)+ `bootstrap_db` 显式初灌;零新落库构造点。
- **守护壳** `e3d-syncd` bin:notify 置脏 + 静定窗(500ms,风暴合并)+ 轮询兜底(30s)+ 单飞串行 + Ctrl-C 优雅退出 + `--once`;触发判定 `round_due` 纯函数单测。
- **守护级实证(真进程两场)**:同字节替换 → Event 轮水位拦截 up-to-date;e3d-writeback 写 ses 37 顶替 → Event 轮 **SYNCED 37..=37** ——「保存即入库」全链(写回→notify→链式增量→幂等落库→水位调度)跑通。
- **验收**:kv-mem 全链测试(基线→幂等→捕获→重启)+ 坏库隔离;T104/T302 双 GATE exit 0;e3d_io 零触碰。
