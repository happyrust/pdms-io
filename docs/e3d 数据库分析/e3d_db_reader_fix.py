...
class E3DDatabase:
    ...
    def _read_metadata(self) -> None:
        ...
        # 使用实际文件大小计算页数
        actual_page_count = self.file_size // page_size
        
        # 读取存储在文件中的页数（从偏移 0x38）
        stored_page_count = struct.unpack('>I', data[0x38:0x3C])[0]
    ...
    def parse_page_header(self, page_data: bytes) -> Dict[str, Any]:
        ...
        if page_type == 5 and len(page_data) >= 8:
            # 数据页面
        ...
        elif page_type == 3 and len(page_data) >= 8:
            # 会话页面
    ...
    def scan_pages(self, page_types: Optional[List[int]] = None) -> Dict[int, List[int]]:
        ...
        for page_num in range(self.metadata['page_count']):
            try:
                ...
                print(f"警告: 读取页面 {page_num} 时出错: {e}")
    ...
    def print_info(self) -> None:
        ...
        print(f"存储页数:      {self.metadata['stored_page_count']}")
