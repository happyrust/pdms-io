...
async fn analyze_scom_15194_4553() {
    ...
    // 打印元素类型
    let noun = map.get_as_string("TYPE").unwrap_or_else(|| "未知".to_string());
    ...
    println!("\n=== 表达式类属性 ===");
    let expr_attrs = [
        "PRAD", "PHEI", "PWID", "PANG", "PDIS", "PDIA", "POFF",
        "DRAD", "DHEI", "DWID", "DANG", "DDIS", "DDIA", "PTCD",
        "XLEN", "YLEN", "ZLEN", "BORE", "OBORE", "IBORE", "FRAD",
    ];
...
async fn analyze_gmse_children() {
    ...
    let (_, offset) = io.search_latest_refno(gmse_refno, None).expect("找不到 GMSE");
                ...
                if let Some(fele_refno) = map.get_foreign_refno("FELE") {
                    ...
                    while let Some(refno) = current {
                        ...
                        if let Some((_, child_offset)) = io.search_latest_refno(refno_u64, None) {
                            let child_record = io.read_element_record_cached(child_offset).unwrap_or_default();
                            let mut child_input = child_record.as_slice();
                            while child_input.len() >= 4 && (child_input[..4] == [0, 0, 0, 0] || child_input[..4] == [0, 0, 0, 7]) {
                    ...
                    while let Some(child_refno) = current {
                        ...
                        if let Some((_, child_offset)) = io.search_latest_refno(child_u64, None) {
                            ...
                            if let Ok(child_data) = parse_ele_data(child_input).await {
                                ...
                                let child_type = child_map.get_as_string("TYPE").unwrap_or_default();
                                ...
                                for (key, val) in child_map.iter() {
                                    ...
                                    let val_upper = val_str.to_uppercase();
                                    if val_upper.contains("TRIM") || val_upper.contains("LOWCASE") {
                                        println!("\n⚠️ 发现目标! 元素 {} ({}):", child_refno, child_type);
                                        println!("   {} = {:?}", key, val);
                                    }
                                }
